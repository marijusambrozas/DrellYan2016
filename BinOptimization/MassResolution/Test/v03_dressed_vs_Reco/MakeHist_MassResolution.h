...
public:
	...
	void Fill(GenPair genpair_preFSR, GenPair genpair_postFSR, Double_t M_Reco, Double_t weight)
	{
		...
		Double_t RelDiff = (M_Reco - M_preFSR) / M_preFSR;
					...
					if( ntuple->isTriggered( analyzer->HLT ) )
					{
						if( this->ChannelType == "MuMu" )
						{
							...
							if( Flag_Matched )
							{
								// genpair_HPFS.Print();
								// mupair.Print();
								Hists->Fill( genpair_dressed, genpair_HPFS, mupair.M, TotWeight );
							}
						}
						else if( this->ChannelType == "EE" )
						{
							...
							if( Flag_Matched )
							{
								...
								// elecpair.Print();
								// cout << endl;
								Hists->Fill( genpair_dressed, genpair_HPFS, elecpair.M, TotWeight );
