...
#define nMassBin 43
#define nMassBin2 86
#define nPtBinEndcap 9
#define nPtBinBarrel 18//14
//#define nPtBin_ele 16
#define nPtBin_ele 10
#define nPtBin_elePR 6
//#define nPtBin_elePR 7
#define nPtBin_DY 16
...
//        const double ptbin_barrel[nPtBinBarrel+1] = {52,55,59,68,85,98,115,130,158,185,220,280,340,440,1500};
//        const double ptbin_endcap[nPtBinEndcap+1] = {52,57,65,76,93,110,165,240,400,1500};
...
//        const double ptbin_ele[nPtBin_ele+1] = {25,30,35,40,45,50,60,70,80,100,120,150,200,300,400,500,5000};
        const double ptbin_ele[nPtBin_ele+1] = {25,33,40,50,65,80,100,140,200,350,1000};
        Double_t FR_barrel_ele[nPtBin_ele];
        Double_t FR_endcap_ele[nPtBin_ele];
        Double_t FR_endcap2_ele[nPtBin_ele];
        const double ptbin_ele_alt[nPtBin_ele_alt+1] = {17, 20, 30, 50, 90, 5000};
        ...
        std::vector<TF1> FRfit_barrel_ele;
        std::vector<TF1> FRfit_endcap_ele;
        std::vector<TF1> FRfit_endcap2_ele;
...
//        const double ptbin_elePR[nPtBin_elePR+1] = {25,32,40,55,80,110,165,600};
        const double ptbin_elePR[nPtBin_elePR+1] = {25,35,50,70,90,130,500};
        ...
        Double_t PR_ele_alt[nPtBin_elePR][3];
        std::vector<TF1> PRfit_barrel_ele;
        std::vector<TF1> PRfit_endcap_ele;
        std::vector<TF1> PRfit_endcap2_ele;
        ...
        // -- Fake Rate method -- //
        Bool_t EventSelection_FR(vector<Muon> MuonCollection, NtupleHandle *ntuple, vector<Muon> *SelectedMuonCollection); // -- output: muons passing numerator and denominator selection -- //
        Bool_t EventSelection_PR(vector<Muon> MuonCollection, NtupleHandle *ntuple, vector<Muon> *SelectedMuonCollection);
        ...
        void SetupFRvalues(TString filename, TString type="sigCtrl_template", TString option="NONE");
        ...
        void SetupPRvalues(TString filename, TString type="ratio", Int_t alt=1, TString option="NONE");
        ...
        Int_t FindTrigger(vector<Muon> MuonCollection, NtupleHandle *ntuple, std::vector<int> *trig_indices, std::vector<int> *trig_fired, std::vector<int> *trig_matched);
        ...
        Int_t FindTrigger(vector<Electron> ElectronCollection, NtupleHandle *ntuple, std::vector<int> *trig_indices, std::vector<int> *trig_fired, std::vector<int> *trig_matched);
        ...
        void SetupFRvalues_ele(TString filename, TString type="subtract", TString option="NONE");
        ...
        void SetupFRvalues_ele_alt2(TString filename, TString type="subtract", TString option="NONE");
        ...
        Double_t FakeRate_ele_alt(Double_t p_T, Double_t eta);
        ...
        void SetupPRvalues_ele_alt(TString filename, TString type="subtract", TString option="NONE");
        void SetupPRvalues_ele_old(TString filename, TString type="subtract", Int_t increaseLowPt=0);
        void SetupPRvalues_ele_fit(TString filename);
        Double_t PromptRate_ele(Double_t p_T, Double_t eta);
        Double_t PromptRate_ele_alt(Double_t p_T, Double_t eta);
        ...
        std::vector<double> MatrixMethod_evtWeight_ele(Double_t pT1, Double_t eta1, Int_t passMediumID1, Double_t pT2, Double_t eta2, Int_t passMediumID2, Int_t alt=0);
        ...
        Double_t GenMuonPt(NtupleHandle* ntuple, Muon reco_mu);
        ...
        void SaveCanvases(std::vector<TCanvas*> &cV, TString destDir, TString type, TFile *fout=NULL);
        // if listOfCanvNames="ALL", saves all canvases
        int  SaveCanvases(TString listOfCanvNames, TString destDir, TString type, TFile *fout=NULL);
};
...
void DYAnalyzer::AssignAccThreshold(TString HLTname, TString *HLT, Double_t *LeadPtCut, Double_t *SubPtCut, Double_t *LeadEtaCut, Double_t *SubEtaCut)
{
        ...
        {
                *HLT = "HLT_Mu17_v* || HLT_Mu20_v* || HLT_Mu27_v* || HLT_Mu50_v*";
                *LeadPtCut = 17;
                *SubPtCut = 17;
                *LeadEtaCut = 2.4;
                *SubEtaCut = 2.4;
                                                ...
                                                vector< Muon >* SelectedMuonCollection) // -- output: 2 muons passing event selection conditions -- //
{
        ...
        {
                Muon mu = QMuonCollection[i_mu];
                if(mu.isTrigMatched(ntuple, this->HLT))
                {
                        isExistHLTMatchedMuon = kTRUE;
                        break;
                }
...
Bool_t DYAnalyzer::EventSelection_FR(vector<Muon> MuonCollection, NtupleHandle *ntuple, vector<Muon>* SelectedMuonCollection)
{
    Bool_t isPassEventSelection = kFALSE;
    Bool_t skip = kTRUE;
    for(Int_t j=0; j<(int)MuonCollection.size(); j++)
    { // Asking for only one tight muon to surpass trigger threshold
        if(MuonCollection[j].Pt > LeadPtCut && fabs(MuonCollection[j].eta) < SubEtaCut && MuonCollection[j].isTightMuon() &&
           MuonCollection[j].isTrigMatched(ntuple, this->HLT))
            skip = kFALSE;
    }
    if (skip == kTRUE)
        return isPassEventSelection;
    ...
    { // All other muons still have to pass these criteria
        if (MuonCollection[j].Pt > SubPtCut && fabs(MuonCollection[j].eta) < SubEtaCut && (MuonCollection[j].isTightMuon()))
        {
            isPassEventSelection = kTRUE;
            SelectedMuonCollection->push_back(MuonCollection[j]);
        }
    }
    return isPassEventSelection;
...
Bool_t DYAnalyzer::EventSelection_PR(vector<Muon> MuonCollection, NtupleHandle *ntuple, vector<Muon>* SelectedMuonCollection)
{
    Bool_t isPassEventSelection = kFALSE;
                                                ...
                                                vector< Electron >* SelectedElectronCollection) // -- output: 2 electrons passing event selection conditions -- //
{
    ...
    for(Int_t j=0; j<(int)ElectronCollection.size(); j++)
    {
        ...
        if(elec.mHits <= 1 && elec.Pt > SubPtCut && fabs(elec.etaSC) < SubEtaCut && !(fabs(elec.etaSC) > 1.4442 && fabs(elec.etaSC) < 1.566))
...
Bool_t DYAnalyzer::EventSelection_FR(vector<Electron> ElectronCollection, NtupleHandle *ntuple, vector<Electron> *SelectedElectronCollection) // Electron selection
{
    ...
    { // Asking for only one electron to surpass trigger threshold
        if(ElectronCollection[j].Pt > LeadPtCut && fabs(ElectronCollection[j].etaSC) < SubEtaCut && ElectronCollection[j].mHits <= 3 &&
    ...
    { // All other electrons still have to pass these criteria
        if (ElectronCollection[j].Pt > SubPtCut && fabs(ElectronCollection[j].etaSC) < SubEtaCut && (ElectronCollection[j].mHits <= 3) &&
           !(fabs(ElectronCollection[j].etaSC) > 1.4442 && fabs(ElectronCollection[j].etaSC) < 1.566))
        {
            isPassEventSelection = kTRUE;
...
void DYAnalyzer::SetupFRvalues(TString filename, TString type, TString option) // type can be "ratio", "template", "mixed", "sigCtrl_template" (default)
{
    ...
    for (Int_t i_bin=1; i_bin<=nPtBinBarrel; i_bin++)
    {
        ...
        if (Option == "UP")
            FR_barrel[i_bin-1] += h_FR_barrel->GetBinError(i_bin);
        else if (Option == "DOWN")
            FR_barrel[i_bin-1] -= h_FR_barrel->GetBinError(i_bin);

        // Checking
        if (FR_barrel[i_bin-1] > 1 || FR_barrel[i_bin-1] < 0)
        {
            nProblem++;
            std::cout << "FR_barrel[" << i_bin-1 << "] = " << FR_barrel[i_bin-1] << endl;
            if (FR_barrel[i_bin-1] > 1) FR_barrel[i_bin-1] = 0.99;
            else FR_barrel[i_bin-1] = 0;
        }
        ...
        if (i_bin <= nPtBinEndcap)
        {
            ...
            if (Option == "UP")
                FR_endcap[i_bin-1] += h_FR_endcap->GetBinError(i_bin);
            else if (Option == "DOWN")
                FR_endcap[i_bin-1] -= h_FR_endcap->GetBinError(i_bin);

            // Checking
            if (FR_endcap[i_bin-1] > 1 || FR_endcap[i_bin-1] < 0)
            {
                nProblem++;
                std::cout << "FR_endcap[" << i_bin-1 << "] = " << FR_endcap[i_bin-1] << endl;
                if (FR_endcap[i_bin-1] > 1) FR_endcap[i_bin-1] = 0.99;
                else FR_endcap[i_bin-1] = 0;
            }

            FR_endcap2[i_bin-1] = h_FR_endcap2->GetBinContent(i_bin);
            if (Option == "UP")
                FR_endcap2[i_bin-1] += h_FR_endcap2->GetBinError(i_bin);
            else if (Option == "DOWN")
                FR_endcap2[i_bin-1] -= h_FR_endcap2->GetBinError(i_bin);

            // Checking
            if (FR_endcap2[i_bin-1] > 1 || FR_endcap2[i_bin-1] < 0)
            {
                nProblem++;
                std::cout << "FR_endcap2[" << i_bin-1 << "] = " << FR_endcap2[i_bin-1] << endl;
                if (FR_endcap2[i_bin-1] > 1) FR_endcap2[i_bin-1] = 0.99;
                else FR_endcap2[i_bin-1] = 0;
            }
        }
    }
...
void DYAnalyzer::SetupPRvalues(TString filename, TString type, Int_t alt, TString option) // type can be "ratio", "template", "mixed", "sigCtrl_template" (default)
{
    ...
    if (!alt)
    {
        for (Int_t i=0; i<nPtBin_DY; i++)
        {
            ...
            if (Option == "UP")
                PR[i][0] += h_PR_barrel->GetBinError(i+1);
            else if (Option == "DOWN")
                PR[i][0] -= h_PR_barrel->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_barrel[" << i << "] = " << PR[i][0] << endl;
                PR[i][0] = 0;
            }
            if (PR[i][0] > 1)
            {
            ...
            if (Option == "UP")
                PR[i][1] += h_PR_barrel2->GetBinError(i+1);
            else if (Option == "DOWN")
                PR[i][1] -= h_PR_barrel2->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_barrel2[" << i << "] = " << PR[i][1] << endl;
                PR[i][1] = 0;
            }
            if (PR[i][1] > 1)
            {
            ...
            if (Option == "UP")
                PR[i][2] += h_PR_endcap->GetBinError(i+1);
            else if (Option == "DOWN")
                PR[i][2] -= h_PR_endcap->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_endcap[" << i << "] = " << PR[i][2] << endl;
                PR[i][2] = 0;
            }
            if (PR[i][2] > 1)
            {
            ...
            if (Option == "UP")
                PR[i][3] += h_PR_endcap2->GetBinError(i+1);
            else if (Option == "DOWN")
                PR[i][3] -= h_PR_endcap2->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_endcap2[" << i << "] = " << PR[i][3] << endl;
                PR[i][3] = 0;
            }
            if (PR[i][3] > 1)
            {
    ...
    else
    {
        for (Int_t i=0; i<nPtBinEndcap; i++)
        {
            ...
            if (Option == "UP")
                PR_alt[i][0] += h_PR_barrel->GetBinError(i+1);
            else if (Option == "DOWN")
                PR_alt[i][0] -= h_PR_barrel->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_barrel[" << i << "] = " << PR_alt[i][0] << endl;
                PR_alt[i][0] = 0;
            }
            if (PR_alt[i][0] > 1)
            {
            ...
            if (Option == "UP")
                PR_alt[i][1] += h_PR_barrel2->GetBinError(i+1);
            else if (Option == "DOWN")
                PR_alt[i][1] -= h_PR_barrel2->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_barrel2[" << i << "] = " << PR_alt[i][1] << endl;
                PR_alt[i][1] = 0;
            }
            if (PR_alt[i][1] > 1)
            {
            ...
            if (Option == "UP")
                PR_alt[i][2] += h_PR_endcap->GetBinError(i+1);
            else if (Option == "DOWN")
                PR_alt[i][2] -= h_PR_endcap->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_endcap[" << i << "] = " << PR_alt[i][2] << endl;
                PR_alt[i][2] = 0;
            }
            if (PR_alt[i][2] > 1)
            {
            ...
            if (Option == "UP")
                PR_alt[i][3] += h_PR_endcap2->GetBinError(i+1);
            else if (Option == "DOWN")
                PR_alt[i][3] -= h_PR_endcap2->GetBinError(i+1);
            ...
            {
                nProblems++;
                std::cout << "PR_endcap2[" << i << "] = " << PR_alt[i][3] << endl;
                PR_alt[i][3] = 0;
            }
            if (PR_alt[i][3] > 1)
            {
...
void DYAnalyzer::SetupFRvalues_ele_fit(TString filename, Int_t relaxedFR)
{
    ...
    TFile *f = new TFile(filename, "READ");
    TF1 *f1_temp;
    FR_func_bins_barrel_ele.push_back(17);
    f->GetObject("f_barrel_25to38", f1_temp);
    FRfit_barrel_ele.push_back(*f1_temp);
    FR_func_bins_barrel_ele.push_back(38);
    f->GetObject("f_barrel_38to180", f1_temp);
    FRfit_barrel_ele.push_back(*f1_temp);
    FR_func_bins_barrel_ele.push_back(180);
    f->GetObject("f_barrel_180to350", f1_temp);
    FRfit_barrel_ele.push_back(*f1_temp);
    FR_func_bins_barrel_ele.push_back(350);
    f->GetObject("f_barrel_350to5000", f1_temp);
    FRfit_barrel_ele.push_back(*f1_temp);
    FR_func_bins_barrel_ele.push_back(5000);

    FR_func_bins_endcap_ele.push_back(17);
    f->GetObject("f_endcap_25to108", f1_temp);
    FRfit_endcap_ele.push_back(*f1_temp);
    FR_func_bins_endcap_ele.push_back(108);
    f->GetObject("f_endcap_108to350", f1_temp);
    FRfit_endcap_ele.push_back(*f1_temp);
    FR_func_bins_endcap_ele.push_back(350);
    f->GetObject("f_endcap_350to5000", f1_temp);
    FRfit_endcap_ele.push_back(*f1_temp);
    FR_func_bins_endcap_ele.push_back(5000);

    FR_func_bins_endcap2_ele.push_back(17);
    f->GetObject("f_endcap2_25to200", f1_temp);
    FRfit_endcap2_ele.push_back(*f1_temp);
    FR_func_bins_endcap2_ele.push_back(200);
    f->GetObject("f_endcap2_200to350", f1_temp);
    FRfit_endcap2_ele.push_back(*f1_temp);
    FR_func_bins_endcap2_ele.push_back(350);
    f->GetObject("f_endcap2_350to5000", f1_temp);
    FRfit_endcap2_ele.push_back(*f1_temp);
    FR_func_bins_endcap2_ele.push_back(5000);

    if (FR_func_bins_barrel_ele.size() != FRfit_barrel_ele.size()+1)
        cout << "ERROR: vector sizes in barrel do not match!\n  Function bins vec size = " << FR_func_bins_barrel_ele.size() <<
                "\n  Functions vector size = " << FRfit_barrel_ele.size() << endl;
    if (FR_func_bins_endcap_ele.size() != FRfit_endcap_ele.size()+1)
        cout << "ERROR: vector sizes in endcap do not match!\n  Function bins vec size = " << FR_func_bins_endcap_ele.size() <<
                "\n  Functions vector size = " << FRfit_endcap_ele.size() << endl;
    if (FR_func_bins_endcap2_ele.size() != FRfit_endcap2_ele.size()+1)
        cout << "ERROR: vector sizes in far endcap do not match!\n  Function bins vec size = " << FR_func_bins_endcap2_ele.size() <<
                "\n  Functions vector size = " << FRfit_endcap2_ele.size() << endl;

    f->Close();
    std::cout << "Setup finished." << endl;

} // End of SetupFRvalues_ele_fit()


void DYAnalyzer::SetupFRvalues_ele(TString filename, TString type, TString option) // type can be "ratio", "template"
{
    TString Option = option;
    Option.ToUpper();
    // -- Setting up -- //
    ...
    {        
        ...
        if (Option == "UP")
            FR_barrel_ele[i_bin-1] += h_FR_barrel->GetBinError(i_bin);
        else if (Option == "DOWN")
            FR_barrel_ele[i_bin-1] -= h_FR_barrel->GetBinError(i_bin);

        // Checking
        if (FR_barrel_ele[i_bin-1] > 1 || FR_barrel_ele[i_bin-1] < 0)
        {
            nProblem++;
            std::cout << "FR_barrel_ele[" << i_bin-1 << "] = " << FR_barrel_ele[i_bin-1] << endl;
            if (FR_barrel_ele[i_bin-1] > 1) FR_barrel_ele[i_bin-1] = 0.99;
            else FR_barrel_ele[i_bin-1] = 0;
        }

        FR_endcap_ele[i_bin-1] = h_FR_endcap->GetBinContent(i_bin);
        if (Option == "UP")
            FR_endcap_ele[i_bin-1] += h_FR_endcap->GetBinError(i_bin);
        else if (Option == "DOWN")
            FR_endcap_ele[i_bin-1] -= h_FR_endcap->GetBinError(i_bin);

        // Checking
        if (FR_endcap_ele[i_bin-1] > 1 || FR_endcap_ele[i_bin-1] < 0)
        {
            nProblem++;
            std::cout << "FR_endcap_ele[" << i_bin-1 << "] = " << FR_endcap_ele[i_bin-1] << endl;
            if (FR_endcap_ele[i_bin-1] > 1) FR_endcap_ele[i_bin-1] = 0.99;
            else FR_endcap_ele[i_bin-1] = 0;
        }

        FR_endcap2_ele[i_bin-1] = h_FR_endcap2->GetBinContent(i_bin);
        if (Option == "UP")
            FR_endcap2_ele[i_bin-1] += h_FR_endcap2->GetBinError(i_bin);
        else if (Option == "DOWN")
            FR_endcap2_ele[i_bin-1] -= h_FR_endcap2->GetBinError(i_bin);

        // Checking
        if (FR_endcap2_ele[i_bin-1] > 1 || FR_endcap2_ele[i_bin-1] < 0)
        {
            nProblem++;
            std::cout << "FR_endcap2_ele[" << i_bin-1 << "] = " << FR_endcap2_ele[i_bin-1] << endl;
            if (FR_endcap2_ele[i_bin-1] > 1) FR_endcap2_ele[i_bin-1] = 0.99;
            else FR_endcap2_ele[i_bin-1] = 0;
        }
    }
...
void DYAnalyzer::SetupFRvalues_ele_alt2(TString filename, TString type, TString option)
{
    TString Option = option;
    Option.ToUpper();
    Int_t nProblem = 0;
    ...
    {
        FR_barrel_ele_alt[i_bin-1] = h_FR_barrel->GetBinContent(i_bin);
        if (Option == "UP")
            FR_barrel_ele_alt[i_bin-1] += h_FR_barrel->GetBinError(i_bin);
        else if (Option == "DOWN")
            FR_barrel_ele_alt[i_bin-1] -= h_FR_barrel->GetBinError(i_bin);

        // Checking
        if (FR_barrel_ele_alt[i_bin-1] > 1 || FR_barrel_ele_alt[i_bin-1] < 0)
        {
            nProblem++;
            std::cout << "FR_barrel[" << i_bin-1 << "] = " << FR_barrel_ele_alt[i_bin-1] << endl;
            if (FR_barrel_ele_alt[i_bin-1] > 1) FR_barrel_ele_alt[i_bin-1] = 0.99;
            else FR_barrel_ele_alt[i_bin-1] = 0;
        }

        FR_endcap_ele_alt[i_bin-1] = h_FR_endcap->GetBinContent(i_bin);
        if (Option == "UP")
            FR_endcap_ele_alt[i_bin-1] += h_FR_endcap->GetBinError(i_bin);
        else if (Option == "DOWN")
            FR_endcap_ele_alt[i_bin-1] -= h_FR_endcap->GetBinError(i_bin);

        // Checking
        if (FR_endcap_ele_alt[i_bin-1] > 1 || FR_endcap_ele_alt[i_bin-1] < 0)
        {
            nProblem++;
            std::cout << "FR_endcap[" << i_bin-1 << "] = " << FR_endcap_ele_alt[i_bin-1] << endl;
            if (FR_endcap_ele_alt[i_bin-1] > 1) FR_endcap_ele_alt[i_bin-1] = 0.99;
            else FR_endcap_ele_alt[i_bin-1] = 0;
        }
    }
    ...
    return;
} // End of SetupFRvalues_ele_alt2()


void DYAnalyzer::SetupFRvalues_ele_old(TString filename, TString type, Int_t increaseLowPt) // type can be "ratio", "template"
{
    // -- Setting up -- //
    std::cout << "Setting up fake rate values from " << filename << endl;
    std::cout << "Fake rate obtained using " << type << " method." << endl;
...
std::vector<double> DYAnalyzer::MatrixMethod_MC(Double_t pT1, Double_t eta1, Double_t relPFiso1, Double_t pT2, Double_t eta2, Double_t relPFiso2, TString PRtype, TString FRtype)
{
    ...
    Double_t FR1 = FakeRate_MC(pT1, eta1, FRtype);//-0.1 < 0 ? 0 : FakeRate(pT1, eta1)-0.1;
    Double_t FR2 = FakeRate_MC(pT2, eta2, FRtype);//-0.1 < 0 ? 0 : FakeRate(pT2, eta2)-0.1;
    Double_t PR1 = PromptRate_MC(pT1, eta1, PRtype);//+0.1 > 1 ? 1 : PromptRate(pT1, eta1)+0.1;
    Double_t PR2 = PromptRate_MC(pT2, eta2, PRtype);//+0.1 > 1 ? 1 : PromptRate(pT2, eta2)+0.1;
...
Double_t DYAnalyzer::FakeRate_ele_fit(Double_t p_T, Double_t eta)
{
    ...
    {
        for (UInt_t i_func=0; i_func<FRfit_barrel_ele.size(); i_func++)
        {
    ...
    else if (fabs(eta) > 1.566 && fabs(eta) < 2.2) // endcap
    {
        for (UInt_t i_func=0; i_func<FRfit_endcap_ele.size(); i_func++)
        {
    ...
    else if (fabs(eta) >= 2.2 && fabs(eta) < 2.4) // far endcap
    {
        for (UInt_t i_func=0; i_func<FRfit_endcap2_ele.size(); i_func++)
        {
    ...
    if (FR == -1) cout << "FR = -1;  p_T = " << p_T <<";  eta = " << eta << endl;
    return FR;
} // End of FakeRate_ele_fit()
...
Double_t DYAnalyzer::FakeRate_ele(Double_t p_T, Double_t eta)
{
    ...
    else if (fabs(eta) >= 2.2 && fabs(eta) < 2.4) // far endcap
    {
        ...
        return FR_endcap2_ele[i_bin];
    }
    return -1;
} // End of FakeRate_ele()


Double_t DYAnalyzer::FakeRate_ele_alt(Double_t p_T, Double_t eta)
{
    Int_t i_bin = 0;
    Int_t stop = 0;
    if (fabs(eta) < 1.4442) // barrel
    {
        while (!stop)
        {
            if (p_T < ptbin_ele_alt[i_bin + 1] || i_bin >= nPtBin_ele_alt-1) // Points exceeding boundaries are assigned last available FR value
                stop = 1;
            else
                i_bin++;
        }
        return FR_barrel_ele_alt[i_bin];
    }
    else if (fabs(eta) > 1.566) // endcap
    {
        ...
        return FR_endcap_ele_alt[i_bin];
    }
    return -1;
} // End of FakeRate_ele_alt()


Double_t DYAnalyzer::FakeRate_ele_old(Double_t p_T, Double_t eta)
{
    Int_t i_bin = 0;
    Int_t stop = 0;
    if (fabs(eta) < 1.4442) // barrel
...
Int_t DYAnalyzer::FindTrigger(vector<Muon> MuonCollection, NtupleHandle *ntuple, std::vector<int> *trig_indices,
                              std::vector<int> *trig_fired, std::vector<int> *trig_matched)
{
    Int_t triggered = 0;
    Int_t trig_index = -1;
...
Int_t DYAnalyzer::FindTrigger(vector<Electron> ElectronCollection, NtupleHandle *ntuple, std::vector<int> *trig_indices,
                              std::vector<int> *trig_fired, std::vector<int> *trig_matched)
{
    Int_t triggered = 0;
    Int_t trig_index = -1;
...
void DYAnalyzer::SetupPRvalues_ele_fit(TString filename)
{
    // -- Setting up -- //
    std::cout << "Setting up prompt rate values from " << filename << endl;
    TFile *f = new TFile(filename, "READ");
    TF1 *f1_temp;
    PR_func_bins_barrel_ele.push_back(17);
    f->GetObject("f_barrel_17to43", f1_temp);
    PRfit_barrel_ele.push_back(*f1_temp);
    PR_func_bins_barrel_ele.push_back(43);
    f->GetObject("f_barrel_43to56", f1_temp);
    PRfit_barrel_ele.push_back(*f1_temp);
    PR_func_bins_barrel_ele.push_back(56);
    f->GetObject("f_barrel_56to95", f1_temp);
    PRfit_barrel_ele.push_back(*f1_temp);
    PR_func_bins_barrel_ele.push_back(95);
    f->GetObject("f_barrel_95to5000", f1_temp);
    PRfit_barrel_ele.push_back(*f1_temp);
    PR_func_bins_barrel_ele.push_back(5000);

    PR_func_bins_endcap_ele.push_back(17);
    f->GetObject("f_endcap_17to42p5", f1_temp);
    PRfit_endcap_ele.push_back(*f1_temp);
    PR_func_bins_endcap_ele.push_back(42.5);
    f->GetObject("f_endcap_42p5to55", f1_temp);
    PRfit_endcap_ele.push_back(*f1_temp);
    PR_func_bins_endcap_ele.push_back(55);
    f->GetObject("f_endcap_55to5000", f1_temp);
    PRfit_endcap_ele.push_back(*f1_temp);
    PR_func_bins_endcap_ele.push_back(5000);

    PR_func_bins_endcap2_ele.push_back(17);
    f->GetObject("f_endcap2_17to42p5", f1_temp);
    PRfit_endcap2_ele.push_back(*f1_temp);
    PR_func_bins_endcap2_ele.push_back(42.5);
    f->GetObject("f_endcap2_42p5to56", f1_temp);
    PRfit_endcap2_ele.push_back(*f1_temp);
    PR_func_bins_endcap2_ele.push_back(56);
    f->GetObject("f_endcap2_56to76", f1_temp);
    PRfit_endcap2_ele.push_back(*f1_temp);
    PR_func_bins_endcap2_ele.push_back(76);
    f->GetObject("f_endcap2_76to5000", f1_temp);
    PRfit_endcap2_ele.push_back(*f1_temp);
    PR_func_bins_endcap2_ele.push_back(5000);

    if (PR_func_bins_barrel_ele.size() != PRfit_barrel_ele.size()+1)
        cout << "ERROR: vector sizes in barrel do not match!\n  Function bins vec size = " << PR_func_bins_barrel_ele.size() <<
                "\n  Functions vector size = " << PRfit_barrel_ele.size() << endl;
    if (PR_func_bins_endcap_ele.size() != PRfit_endcap_ele.size()+1)
        cout << "ERROR: vector sizes in endcap do not match!\n  Function bins vec size = " << PR_func_bins_endcap_ele.size() <<
                "\n  Functions vector size = " << PRfit_endcap_ele.size() << endl;
    if (PR_func_bins_endcap2_ele.size() != PRfit_endcap2_ele.size()+1)
        cout << "ERROR: vector sizes in far endcap do not match!\n  Function bins vec size = " << PR_func_bins_endcap2_ele.size() <<
                "\n  Functions vector size = " << PRfit_endcap2_ele.size() << endl;

    f->Close();
    std::cout << "Setup finished." << endl;
} // End of SetupPRvalues_ele_fit()
...
void DYAnalyzer::SetupPRvalues_ele(TString filename, TString type, Int_t increaseLowPt)
{
    ...
    for (Int_t i=0; i<nPtBin_DY; i++)
    {
        ...
        PR_ele[i][2] = h_PR_endcap2->GetBinContent(i+1);
        if (PR_ele[i][2] < 0)
        {
            nProblems++;
            std::cout << i << "  " << PR_ele[i][2] << endl;
            PR_ele[i][2] = 0;
        }
        if (PR_ele[i][2] > 1)
        {
...
void DYAnalyzer::SetupPRvalues_ele_alt(TString filename, TString type, TString option)
{
    TString Option = option;
    Option.ToUpper();
    cout << "Setting up PR values...";
    ...
    for (Int_t i=0; i<nPtBin_elePR; i++)
    {        
        PR_ele_alt[i][0] = h_PR_barrel->GetBinContent(i+1);
        if (Option == "UP")
            PR_ele_alt[i][0] += h_PR_barrel->GetBinError(i+1);
        else if (Option == "DOWN")
            PR_ele_alt[i][0] -= h_PR_barrel->GetBinError(i+1);
        ...
        {
            nProblems++;
            std::cout << "PR_barrel[" << i << "] = " << PR_ele_alt[i][0] << endl;
            PR_ele_alt[i][0] = 0;
        }
        if (PR_ele_alt[i][0] > 1)
        {
        ...
        PR_ele_alt[i][1] = h_PR_endcap->GetBinContent(i+1);
        if (Option == "UP")
            PR_ele_alt[i][1] += h_PR_endcap->GetBinError(i+1);
        else if (Option == "DOWN")
            PR_ele_alt[i][1] -= h_PR_endcap->GetBinError(i+1);
        ...
        {
            nProblems++;
            std::cout << "PR_endcap[" << i << "] = " << PR_ele_alt[i][1] << endl;
            PR_ele_alt[i][1] = 0;
        }
        if (PR_ele_alt[i][1] > 1)
        {
        ...
        PR_ele_alt[i][2] = h_PR_endcap2->GetBinContent(i+1);
        if (Option == "UP")
            PR_ele_alt[i][2] += h_PR_endcap2->GetBinError(i+1);
        else if (Option == "DOWN")
            PR_ele_alt[i][2] -= h_PR_endcap2->GetBinError(i+1);
        ...
        {
            nProblems++;
            std::cout << "PR_endcap2[" << i << "] = " << PR_ele_alt[i][2] << endl;
            PR_ele_alt[i][2] = 0;
        }
        if (PR_ele_alt[i][2] > 1)
        {
    ...
    if (nProblems)
        std::cout << "**************************************************\n" <<
                     "Problems were detected: prompt rate values exceeding 'regular' probability boundaries have been found.\n" <<
                     "Please check the code.\n**************************************************" << endl;
} // End of SetupPRvalues_ele_alt()


void DYAnalyzer::SetupPRvalues_ele_old(TString filename, TString type, Int_t increaseLowPt)
{
    cout << "Setting up PR values...";
...
Double_t DYAnalyzer::PromptRate_ele_fit(Double_t p_T, Double_t eta)
{
    Double_t PR = -1;
    if (fabs(eta) < 1.4442) // barrel
    {
        for (UInt_t i_func=0; i_func<PRfit_barrel_ele.size(); i_func++)
        {
    ...
    else if (fabs(eta) > 1.566 && fabs(eta) < 2.1) // endcap
    {
        for (UInt_t i_func=0; i_func<PRfit_endcap_ele.size(); i_func++)
        {
    ...
    else if (fabs(eta) >= 2.1 && fabs(eta) < 2.4) // far endcap
    {
    ...
    if (PR == -1) cout << "PR = -1;  p_T = " << p_T <<";  eta = " << eta << endl;
    return PR;
} // End of PromptRate_ele_fit()
...
Double_t DYAnalyzer::PromptRate_ele(Double_t p_T, Double_t eta)
{
    ...
    else if (fabs(eta) >= 2.1 && fabs(eta) < 2.4) // endcap
    {
        etabin = 2;
        while (!stop)
        {
...
Double_t DYAnalyzer::PromptRate_ele_alt(Double_t p_T, Double_t eta)
{
    Double_t PR = 1;
    ...
    if (fabs(eta) < 1.4442) // barrel
    {
        ...
        while (!stop)
        {
            if (p_T < ptbin_elePR[i_bin + 1] || i_bin >= nPtBin_elePR-1) // Points exceeding boundaries are assigned last available value
                stop = 1;
            else
                i_bin++;
        }
    ...
    PR = PR_ele_alt[i_bin][etabin];

    return PR;
} // end of PromptRate_ele_alt



Double_t DYAnalyzer::PromptRate_ele_old(Double_t p_T, Double_t eta)
{
    Double_t PR = 1;
...
std::vector<double> DYAnalyzer::MatrixMethod_evtWeight_ele_fit(Double_t pT1, Double_t eta1, Int_t passMediumID1, Double_t pT2, Double_t eta2, Int_t passMediumID2)
{
    ...
    Double_t FR1 = FakeRate_ele_fit(pT1, eta1);//-0.2 < 0 ? 0 : FakeRate_ele_fit(pT1, eta1)-0.2;
    Double_t FR2 = FakeRate_ele_fit(pT2, eta2);//-0.2 < 0 ? 0 : FakeRate_ele_fit(pT2, eta2)-0.2;
    Double_t PR1 = PromptRate_ele_fit(pT1, eta1);//-0.2 < 0 ? 0 : PromptRate_ele_fit(pT1, eta1)-0.2;
    Double_t PR2 = PromptRate_ele_fit(pT2, eta2);//-0.2 < 0 ? 0 : PromptRate_ele_fit(pT2, eta2)-0.2;
...
std::vector<double> DYAnalyzer::MatrixMethod_evtWeight_ele(Double_t pT1, Double_t eta1, Int_t passMediumID1, Double_t pT2, Double_t eta2, Int_t passMediumID2, Int_t alt)
{
    std::vector<double> weights(4, 1.0);
    ...
    if (!alt)
    {
        PR1 = PromptRate_ele(pT1, eta1);
        PR2 = PromptRate_ele(pT2, eta2);
    }
    else
    {
        PR1 = PromptRate_ele_alt(pT1, eta1);
        PR2 = PromptRate_ele_alt(pT2, eta2);
    }
...
Double_t DYAnalyzer::GenMuonPt(NtupleHandle* ntuple, Muon reco_mu)
{
...
void DYAnalyzer::SaveCanvases(std::vector<TCanvas*> &cV, TString destDir, TString type, TFile *fout) // Copied from Andrius Juodagalvis
// Updated by Marijus Ambrozas
{
    if (cV.size()==0 && !fout) {
    ...
    if (!type.Length() && !fout) {
        std::cout << "SaveCanvases: no type\n";
        return;
    }
    type.ToUpper();
    if (!type.Contains("PDF") && !type.Contains("PNG") && !type.Contains("ROOT") && !fout) {
        std::cout << "SaveCanvases: wrong type\n";
        return;
    }
    ...
    if (destDir.Length())
    {
        gSystem->Exec("mkdir "+destDir);
        if (type.Contains("PNG"))
            gSystem->Exec("mkdir "+destDir+TString("/png"));
        if (type.Contains("PDF"))
            gSystem->Exec("mkdir "+destDir+TString("/pdf"));
        if (type.Contains("ROOT"))
            gSystem->Exec("mkdir "+destDir+TString("/root"));
...
int DYAnalyzer::SaveCanvases(TString listOfCanvNames, TString destDir, TString type, TFile *fout) // Copied from Andrius Juodagalvis
{
    ...
    if (listOfCanvNames=="ALL")
    {
        ...
        SaveCanvases(cV,destDir,type,fout);
        return 1;
    }
    ...
    if (cV.size()) SaveCanvases(cV,destDir,type,fout);
    return int(cV.size());
}
