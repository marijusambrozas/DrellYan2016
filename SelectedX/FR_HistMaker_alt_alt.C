...
void FR_HistMaker_alt_alt (TString WhichX = "", Int_t type=1)
{
...
//            }
            ...
            for (UInt_t i_ele=0; i_ele<p_T->size(); i_ele++)
            {
                ...
                Int_t matched_photon = 0, matched_ele = 0;
                prescale_alt = 0;

                for (UInt_t i_tr=0; i_tr<trig_fired->size(); i_tr++)
                {
                    if (trig_fired->at(i_tr) == 2312)
                    {
                ...
                if (matched_ele == 0 || matched_photon == 0) continue;
