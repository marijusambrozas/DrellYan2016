...
//                            L1_trigPS->push_back(ntuple->L1_trigPS->at(i_L1));
                        ...
                        for (Int_t i_ele=0; i_ele<nElectrons; i_ele++)
                        {
                            ...
                            if (Debug == kTRUE)
                            {
                                ...
                                cout << endl;
                                if ((UInt_t)i_ele < trig_fired->size())
...
void MakeSelectionForFR_Mu (TString type, TString HLTname, Bool_t Debug)
{
    ...
    for (Int_t i_proc=0; i_proc<Nproc; i_proc++)
    {
        ...
        MuonTree->Branch("LHE_px", &LHE_px);
        MuonTree->Branch("LHE_py", &LHE_py);
        MuonTree->Branch("LHE_pz", &LHE_pz);
        MuonTree->Branch("LHE_E",  &LHE_E );
        MuonTree->Branch("LHE_ID", &LHE_ID);
        MuonTree->Branch("LHE_status", &LHE_status);
